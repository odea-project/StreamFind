...
rt2 <- data.frame(rtmin = c(809, 907), rtmax = c(869, 967))
...
peaks(exS2_pp[c(1, 4)], mz = mz1[1], rt = rt1[1], ppm = 10, sec = 30)

#access to peak in msAnalysis
peaks(exA2, mz = targets4)

#plotting peaks in msData
plotPeaks(exS2_pp, mz = targets4[2, ], interactive = TRUE, colorBy = "analyses")

#plotting peaks from msAnalysis
plotPeaks(exA2, mz = targets4, interactive = FALSE)

#map plot for peak time and m/z dimensions for msAnalysis
mapPeaks(exA2, mz = targets4[1, ], xlim = 30, ylim = 0.001)

##map plot for peak time and m/z dimensions for msData
mapPeaks(exS2_pp, mz = targets4[2, ], colorBy = "replicates", xlim = 30, ylim = 0.001)
```
...
features(exS2_pg, mz = targets4)

#getter for all the feature metadata by setting complete to TRUE
t(features(exS2_pg_2, mz = targets4[1, ], complete = TRUE))

#getter for features with intensities for each sample by setting average to FALSE
features(exS2_pg_2, mz = targets4[1, ], average = FALSE)

#plotting features
plotFeatures(exS2_pg, mz = targets4, colorBy = "targets", interactive = FALSE)

#plotting features for replicates and interactive
plotFeatures(exS2_pg, mz = targets4, colorBy = "replicates", interactive = TRUE)

#plot the individual peaks in features and for each analysis shows the time deviation.
plotFeaturePeaks(exS2_pg, mz = targets4)
```
