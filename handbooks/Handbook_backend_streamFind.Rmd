...
df_files_desc <- data.frame(
  ...
  polarity = c(rep("positive", 6),
               ...
               rep("negative", 3), rep("positive",3)),
  description = c(
    rep("Basic centroided MS data spiked with chemical and internal standards (CS and IS, respectively).", 3),
    rep("Basic profile MS data spiked with CS and IS.", 3),
    rep("Blank MS data spiked with IS.", 6),
    rep("Wastewater secondary effluent MS data spiked with IS.", 6),
    rep("Wastewater secondary effluent treated with ozonated strong water MS data spiked with IS.", 6)
  )
)
...
knitr::kable(db[, .(name, CAS, formula, neutralMass, rt, ionization, tag)], caption = "Chemical standards spiked.") %>%
...
t(streamFind::metadata(sp1, analyses = 4))
...
metadata(a1, which = names(meta_vec))
...
metadata(sp1, which = colnames(meta_df_2))
```
...
xic1 <- XICs(sp1, analyses = 3:4, mz = mz1[2], rt = rt1[2], ppm = 50, sec = 30)
xic1
```
...
sp2 <- newStreamSet(files = files[c(10:12, 16:18)], path = getwd(), title = "Project Example")
```
...
#access to peaks in msData based on built targets as used for extracting EICs
pks_1 <- peaks(sp2_pp[c(1, 4)], mz = mz1[1], rt = rt1[1], ppm = 10, sec = 30)
