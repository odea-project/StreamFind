...
The streamFind package includes example files, which are located  in the folder `system.file(package = "streamFind", dir = "extdata")`. Note, the MS files (.mzML) were trimmed with the function `trimSpectraFilesMZR` to reduce their size. The files available are described in Table \@ref(tab:description-files).
...
df_files_desc <- data.frame(
  ...
  description = c(
    ...
    rep("Blank centroided MS data spiked with internal standards.", 3),
    rep("Wastewater secondary effluent centroided MS data spiked with internal standards.", 3)
  )
)
...
`r # TODO add a list of spiked chemical in the package` List the chemicals spiked in the MS data files.

# Objects

The streamFind package uses S4 class objects. The S4 classes, main methods and usage are presented and exemplified in this section. An overview of the S4 classes and their hierarchy is presented in Figure \@ref(fig:classes-overview) using MS analyses as an example. Other data types, such as from UV and Raman analyses, would lead to different classes (e.g., _uvData_ and _uvAnalysis_ and _ramanData_ and _ramanAnalysis_, respectively) but the hierarchy is similar as shown for MS data.

```{r classes-overview, echo=FALSE, fig.cap="Overview of the S4 classes and their hierarchy with emphasis on MS analyses.", out.width="120%"}
knitr::include_graphics(paste0(getwd(), "/hb_backend_figures/classes_overview.png"))
```

## Basic terminalogy

In streamFind, the terms listed below are consistently applied throughout the objects and functionalities.

* __project__ is the project title;
* __analysis__ is the name or the actual analysis file;
* __replicate__ is the name of an analysis replicate group;
* __blank__ is the name of a blank analysis replicate group ot be used for blank (blind) subtration;
* __trace__ correspond to a raw data signal (e.g., mass trace in MS data) obtained from a given analysis file;  
* __spectra__ in MS is a table with a spectrum for each cycle time (i.e., retention time);
* __EIC__ is an extracted ion chromatograms (retention time vs intensity);
* __TIC__ is a total ion chromatograms (retention time vs intensity);
* __XIC__ is a three dimensional ion chromatogram (_m/z_ vs retention time vs intensity);
...
* __MS2__ correspond to second order fragmentation data as acquired in tandem MS;
(...)
...
t(streamFind::metadata(exS1, analyses = 1))

#applied directly to the msAnalysis S4 class
t(streamFind::metadata(exA1))
```
...
t(streamFind::annotation(exS2_pg_fl_an, mz = 441.1670, ppm = 10, rt = 916, sec = 5, all = FALSE))
...
target_s <- streamFind::annotation(exS2_pg_fl_an, mz = 441.1670, ppm = 10, rt = 916, sec = 5, all = FALSE)
...
exS2_pg_fl_an_filFeat <- addParameters(exS2_pg_fl_an, where = "features", settings = settings_filFeat)

exS2_pg_fl_an_filFeat <- filterFeatures(exS2_pg_fl_an_filFeat)

#number of features not filtered
nrow(features(exS2_pg_fl_an, filtered = FALSE))

#number of features not filtered after applying the filters
nrow(features(exS2_pg_fl_an_filFeat, filtered = FALSE))

#remove permanently all filtered features from the msData object
exS2_pg_fl_an_filFeat_rem <- removeFilteredFeatures(exS2_pg_fl_an_filFeat, which =  "all")

exS2_pg_fl_an_filFeat_rem
```
...
```{r exTable, echo=TRUE, eval=TRUE, include=TRUE}
