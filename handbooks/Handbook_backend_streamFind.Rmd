...
data.table::setnames(db, "mix_id", "tag", skip_absent = TRUE)
db[tag %in% 1, tag := "CS"]
db[, in_file := "1-6"]
db[tag %in% "IS", in_file := "1-24"]

knitr::kable(db[, .(name, formula, mass, rt, ionization, tag, in_file)], caption = "Chemical standards spiked.") %>%
