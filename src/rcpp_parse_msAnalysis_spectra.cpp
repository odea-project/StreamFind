...
Rcpp::List rcpp_parse_msAnalysis_spectra(Rcpp::List analysis, Rcpp::IntegerVector which = NA_INTEGER) {
  ...
  list_output.attr("class") = Rcpp::CharacterVector::create("data.table", "data.frame");
  ...
  if (result) {
    ...
    Rcpp::IntegerVector index =  run["index"];
    ...
    if (index.size() > 0) {
      ...
      const Rcpp::IntegerVector& scan = run["scan"];
      ...
      // return only selected scans, by adapting the index vector
        ...
        // TODO sort the which vector;

        // remove any larger than the number_spectra
        Rcpp::LogicalVector overhead = which <= index.size();
        which = which[overhead];

        // remove duplicated indexes
        std::set<int> which_set(which.begin(), which.end());
        Rcpp::IntegerVector index_unique(which_set.begin(), which_set.end());

        bins = xml_utils::parse_partial_spectra(root, index_unique);
