...
Rcpp::List rcpp_nts_annotate_features(Rcpp::List feature_list,
                                     ...
                                     int maxGaps = 1)
{
  NTS::ANNOTATION_ISOTOPE_SET isotopes;
  ...
  NTS::ANNOTATION_ISOTOPE_COMBINATIONS combinations(isotopes, max_number_elements);
  Rcpp::Rcout << "Done!" << std::endl;
  ...
  for (int a = 0; a < number_analyses; a++)
  {
    ...
    {
      NTS::FEATURE main_ft = fts.get_feature(f);

      if (main_ft.annotation.iso_cat != "")
        continue;

      NTS::ANNOTATION_CANDIDATE_CHAIN candidates_chain;
    ...
    for (int f = 0; f < number_features; f++)
    {
      ...
      NTS::ANNOTATION_CANDIDATE_CHAIN adduct_candidates_chain;
                                  ...
                                  float minGaussianFit = 0.5)
{
  ...
  if (!data.valid)
  {
    Rcpp::Rcout << "Error: Invalid NTS data! Not filled." << std::endl;
    return feature_list;
  }
  ...
  for (int i = 0; i < number_analyses; i++)
  {
    const NTS::FEATURES &fts_i = data.features[i];
  ...
  for (int j = 0; j < number_analyses; j++)
  {
    ...
    for (int i = n_j_targets - 1; i >= 0; --i)
    {
      ...
      NTS::FEATURES &fts_j = data.features[j];
