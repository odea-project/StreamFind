...
Rcpp::List rcpp_parse_xml(Rcpp::NumericVector input) {
 ...
 // decompress with zlib
  ...
  // // // From numeric to little_endian binary as RawVector
  ...
  // // // From numeric to big_endian binary

  // std::vector<uint8_t> data;
  //
  // for (int i = 0; i < input.size(); i++) {
  //   uint64_t value = *reinterpret_cast<const uint64_t*>(&input[i]);
  //   std::vector<uint8_t> bytes(8);
  //   for (int j = 0; j < 8; j++) {
  //     bytes[j] = (value >> ((7 - j) * 8)) & 0xFF;
  //   }
