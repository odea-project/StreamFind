...
AuditTrail::AuditTrail(std::shared_ptr<Context> ctx) : TableBase<AuditTrailRow>(std::move(ctx)) {
...
void AuditTrail::create_schema(const std::shared_ptr<Context>& ctx) {
...
void AuditTrail::validate_schema(const std::shared_ptr<Context>& ctx) {
  auto guard = project::detail::ConnectionGuard(ctx);
...
std::vector<AuditTrail::Row> AuditTrail::all() const {
...
void AuditTrail::add(const std::string& operation_type,
