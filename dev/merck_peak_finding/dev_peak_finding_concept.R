...
#plot_3D_by_rt(sp_clustered, c(1:50))
...
source("dev/dev_peak_finding/dev_generate_formulas.R")
formulas_targets <- generate_peak_formulas(
  peaks_dt[peaks_dt$id %in% matched_targets$peak_id, ],
  mzabs = 0.005,
  ppm = 5,
  elements = list(c("C", 20, 100), c("H", 20, 100), c("N", 0, 3)),
  validation = FALSE,
  charge = FALSE
)
