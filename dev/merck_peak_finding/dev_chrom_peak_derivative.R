...
find_peaks <- function(sp_clustered, parameters) {
  ...
  peaks_list <- pbapply::pblapply(spl_clusters, function(temp_spec, parameters) {
    ...
    peaks <- peak_detect_derivative(temp_spec, parameters$minTraces, parameters$baselineWindow, parameters$maxWidth, parameters$minSNR, parameters$minGaussFit, plotPeaks = FALSE)
  ...
  peaks_dt[["rt"]] <- round(peaks_dt[["rt"]], 0)
  peaks_dt[["mz"]] <- round(peaks_dt[["mz"]], 4)
  peaks_dt[["mz_wt"]] <- round(peaks_dt[["mz_wt"]], 4)
  peaks_dt[["mz_appex"]] <- round(peaks_dt[["mz_appex"]], 4)
  peaks_dt[["intensity"]] <- round(peaks_dt[["intensity"]], 0)
  peaks_dt[["width"]] <- round(peaks_dt[["width"]], 0)
  peaks_dt[["ppm"]] <- round(peaks_dt[["ppm"]], 2)
  peaks_dt[["mzmin"]] <- round(peaks_dt[["mzmin"]], 4)
  peaks_dt[["mzmax"]] <- round(peaks_dt[["mzmax"]], 4)
  peaks_dt[["rtmin"]] <- round(peaks_dt[["rtmin"]], 0)
  peaks_dt[["rtmax"]] <- round(peaks_dt[["rtmax"]], 0)
  peaks_dt[["noise"]] <- round(peaks_dt[["noise"]], 0)
  peaks_dt[["sn"]] <- round(peaks_dt[["sn"]], 2)
  peaks_dt[["A"]] <- round(peaks_dt[["A"]], 2)
  peaks_dt[["mu"]] <- round(peaks_dt[["mu"]], 0)
  peaks_dt[["sigma"]] <- round(peaks_dt[["sigma"]], 2)
  peaks_dt[["r_squared"]] <- round(peaks_dt[["r_squared"]], 4)
  peaks_dt[["n_traces"]] <- as.integer(peaks_dt[["n_traces"]])
  peaks_dt[["mz_sd"]] <- round(peaks_dt[["mz_sd"]], 4)
