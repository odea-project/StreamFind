#' @title MassSpecMethod_CorrectSpectraBaseline_baseline_als Class
#'
...
#' @export
#'
MassSpecMethod_CorrectSpectraBaseline_baseline_als <- function(
  lambda = 5,
  p = 0.05,
  maxit = 10
) {
  ...
  if (is.null(validate_object(x))) {
    ...
    stop("Invalid MassSpecMethod_CorrectSpectraBaseline_baseline_als object!")
  }
}
...
validate_object.MassSpecMethod_CorrectSpectraBaseline_baseline_als <- function(
  ...
  checkmate::test_choice(x$type, "MassSpec")
  checkmate::test_choice(x$method, "CorrectSpectraBaseline")
  checkmate::test_choice(x$algorithm, "baseline_als")
...
#' @export
#' @noRd
run.MassSpecMethod_CorrectSpectraBaseline_baseline_als <- function(
  x,
  engine = NULL
) {
...
#' @title MassSpecMethod_CorrectSpectraBaseline_airpls Class
#'
...
#' @export
#'
MassSpecMethod_CorrectSpectraBaseline_airpls <- function(
  lambda = 10,
  differences = 1,
  itermax = 20
) {
  ...
  if (is.null(validate_object(x))) {
    ...
    stop("Invalid MassSpecMethod_CorrectSpectraBaseline_airpls object!")
  }
}
...
#' @export
#' @noRd
run.MassSpecMethod_CorrectSpectraBaseline_airpls <- function(x, engine = NULL) {
      ...
      if (nrow(z) > 0) {
        if ("rt" %in% colnames(z)) {
          ...
          temp_x <- lapply(temp_x, function(z) {
            ...
            z$raw <- z$intensity
            baseline_data[baseline_data > z$intensity] <- z$intensity[
              baseline_data > z$intensity
            ]
