...
```{r tics, message=FALSE, fig.cap="Total ion chromatogram (TIC) of all analyses by replicate group name."}
...
```{r internal-standards-eic, fig.height=7, fig.cap="Spiked internal standards after feature finding and grouping.", message=FALSE}
ms$plot_groups(ms$get_internal_standards()[,c("group", "name"), with = FALSE], legendNames = TRUE, colorBy = "targets+replicates")
```
...
```{r internal-standards-eic-old, include=FALSE, fig.height=7, fig.cap="Extracted ion chromatogram (EIC) of four spiked internal standards per analysis replicate group.", message=FALSE}
# ms$get_internal_standards(average = FALSE)
# ms$plot_eic(mass = dbis[, c("name", "mass"), with = FALSE], colorBy = "replicates+targets", legendNames = TRUE, ppm = 20)
  ...
  anaMpList <- lapply(anaInfo$analysis, function(x, blkGroups, blkAnalyses, tics, rtWindow) {
    ...
    mpListX <- lapply(tics[blkA], function(z, ticX, rtWindow) {
      ...
      mp <- vapply(seq_len(nrow(ticX)), function(j, z) {
        ...
        intX <- mean(ticX$int[.trimVector(ticX$rt, rtr[1], rtr[2])])
        
        # eq. 6 from 10.1021/acs.analchem.1c00357
        (intX / intB) * -1
        
        # alternative using the intensity at the exact rt
        # (ticX$int[j] / intB) * -1
...
calculateCorrectedIntensities <- function(dt, anaInfo, ticMps, rtWindow) {
  ...
  for (i in seq_len(nrow(out))) {
    ...
    if (!any(selBlkInts)) {
      ...
      # What to do?
      # Find another feature in the near retention time?
      # Pre-calculate retention time and mass based scales? 
...
rpls <- ms$get_replicate_names()

corrInts <- calculateCorrectedIntensities(dt, anaInfo, ticMps, rtWindow = 4)
corrInts$replicate <- rpls[corrInts$analysis]
...
dt2 <- ms$get_features(mass = db, sec = 15, ppm = 10)
ms$plot_groups_overview(groups = dt2, legendNames = TRUE)

ms$plot_eic(mass = db, legendNames = TRUE)

corrInts2 <- calculateCorrectedIntensities(dt2, anaInfo, ticMps, rtWindow = 4)

library(ggplot2)

ggplot(corrInts, aes(x=rt, y=scale, shape=replicate, color=group)) +
  geom_point()

ggplot(corrInts, aes(x=rt, y=ticmp, shape=replicate, color=group)) +
  geom_point()

ggplot(corrInts, aes(x=mp, y=ticmp, shape=replicate, color=group)) +
  geom_point()


fig <- plot_ly(corrInts, x = corrInts$rt)

fig <- fig %>% add_trace(corrInts,
  x = corrInts$rt,
  y = corrInts$scale,
  color = corrInts$replicate,
  name = corrInts$replicate,
  legendgroup = corrInts$replicate,
  type = "scatter", mode = "markers",
  marker = list(size = 10)
)

fig2 <- plot_ly(corrInts, x = corrInts$rt)

fig2 <- fig2 %>% add_trace(corrInts,
  x = corrInts$rt,
  y = corrInts$ticmp,
  color = corrInts$replicate,
  name = corrInts$replicate,
