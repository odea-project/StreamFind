...
ms <- ms$subset_analyses(analyses = c(1:63, 73:99, 118:121, 123:130, 134:170))

ms$add_blank_names(c(rep("blank", 94), rep("blank_e", 139 - 94)))
...
ms$plot_spectra_tic(analyses = ov$analysis[ov$replicate %in% "blank_e"], colorBy = "analyses", interactive = TRUE)
```
...
kable(data.frame(replicate = rownames(train_set), class = labels), caption = "Class assignment for training set.") %>%
kable_styling(font_size = 12, bootstrap_options = c("striped", "hover", "condensed", "responsive"), fixed_thead = TRUE) %>%
scroll_box(width = "100%", height = "400px")
```
...
ov_quant$concentration <- as.numeric(c(rep(0, 9), sub(".*?(\\d+).*", "\\1", ov_quant$replicate[10:nrow(ov2)]))) / 10
