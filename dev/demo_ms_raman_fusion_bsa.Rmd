...
ms$run(MassSpecMethod_LoadChromatograms_StreamFind(chromatograms = "TIC"))
...
According to the MS1 spectra, the *m/z* from 1200 to 2000 can be added to the dedicated *ProcessingStep* for deconvolution, as shown below. The retention time dimensions for extraction of spectra is based on the local peak maxima shown above.
...
ms$run(MassSpecMethod_DeconvoluteSpectra_StreamFind())
...
```{r raman-workflow}
raman_workflow <- list(
  RamanMethod_BinScans_StreamFind(
    binNames = "rt",
    binValues = 20,
    byUnit = FALSE
  ),

  RamanMethod_SubtractSpectraSection_native(
    sectionVal = "rt",
...
stat$run(StatisticMethod_MakeModel_mcrpure_mdatools(ncomp = 2, offset = 0.05))
summary(stat$model)
```
