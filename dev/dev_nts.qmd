...
ms$NTS$feature_list[[11]]

View(fl[, c("feature", "annotation")])


plot_suspects(ms$NTS, analyses = 11)

get_suspects(ms$NTS, analyses = 11)$fragments


fl <- rcpp_nts_load_features_ms2(
  ms$NTS$analyses_info[1, ],
  ms$NTS$spectra_headers[1],
...
plot_groups_overview(ms$NTS, mass = dbis, legendNames = TRUE)


plot_features_ms2(ms$NTS, analyses = 11, mass = dbis, legendNames = TRUE, filtered = TRUE)

plot_features(ms$NTS, analyses = 4, mass = dbis, legendNames = TRUE)

map_components(ms$NTS, analyses = 4, mass = dbis, legendNames = TRUE)


get_features(ms$NTS, analyses = 4, mass = dbis)


fl_temp <- ms$NTS$feature_list[4:5]
fl_temp <- lapply(fl_temp, function(z) z[order(mz, decreasing = TRUE), ])


res <- rcpp_nts_fill_features(
  ms$NTS$analyses_info,
  ms$NTS$spectra_headers,
  ms$NTS$feature_list,
  withinReplicate = FALSE,
  filtered = FALSE,
  rtExpand = 2,
...
#   ms$get_files()[1:2],
#   ms$NTS@spectra_headers[1:2],
#   ms$NTS$feature_list[1:2],
#   TRUE,
#   rtExpand = 2,
#   mzExpand = 0.002,
#   minPeakWidth = 10,
