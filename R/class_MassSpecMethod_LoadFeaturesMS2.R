...
#' @title MassSpecMethod_LoadFeaturesMS2_native class
...
#' @export
#'
MassSpecMethod_LoadFeaturesMS2_native <- function(
  isolationWindow = 1.3,
  mzClust = 0.005,
  presence = 0.8,
  minIntensity = 10,
  ...
  if (is.null(validate_object(x))) {
    ...
    stop("Invalid parameters for MassSpecMethod_LoadFeaturesMS2_native.")
  }
}

#' @export
#' @noRd
validate_object.MassSpecMethod_LoadFeaturesMS2_native <- function(x) {
...
#' @export
#' @noRd
run.MassSpecMethod_LoadFeaturesMS2_native <- function(x, engine = NULL) {
  ...
  fts <- rcpp_nts_load_features_ms2(
    info = analyses,
  ...
  if (!is.null(cache_manager)) {
    save_cache(
      cache_manager,
      name = paste0("LoadFeaturesMS2_native"),
      hash = .make_hash(x, analyses, parameters, engine$Workflow),
      description = "MS2 loaded with LoadFeaturesMS2_native method",
