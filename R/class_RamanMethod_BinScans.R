...
#' @export
#'
RamanMethod_BinScans_native <- function(
  mode = c("unit"),
  value = 5,
  global = FALSE,
  ...
  if (is.null(validate_object(x))) {
    ...
    stop("Invalid RamanMethod_BinScans_native object!")
  }
}
...
validate_object.RamanMethod_BinScans_native <- function(x) {
  ...
  checkmate::assert_choice(x$parameters$mode, c("scans", "time"))
  checkmate::assert_numeric(x$parameters$value, max.len = 1)
  checkmate::assert_logical(x$parameters$global, len = 1)
  checkmate::assert_numeric(x$parameters$refAnalysis, len = 1)
  NextMethod()
  NULL
}
...
#' @export
#' @noRd
run.RamanMethod_BinScans_native <- function(x, engine = NULL) {
  ...
  spec_binned <- lapply(spec_list, function(z) {
    ...
    if ("scans" %in% binning_mode) {
      ...
      bin_key <- cut(
        urts,
        seq(min(urts), max(urts), binning_value),
        include.lowest = TRUE
      )
