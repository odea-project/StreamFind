...
#' @export
#'
MassSpecMethod_CreateComponents_native <- function(
	rtWindow = c(0, 0),
	minCorrelation = 0.8,
	debugRT = 0,
	debugAnalysis = ""
	...
	if (is.null(validate_object(x))) {
		...
		stop("Invalid parameters for MassSpecMethod_CreateComponents_native.")
	}
}

#' @export
#' @noRd
validate_object.MassSpecMethod_CreateComponents_native <- function(x) {
...
#' @export
#' @noRd
run.MassSpecMethod_CreateComponents_native <- function(x, engine = NULL) {
	...
	if (!is.null(cache_manager)) {
		save_cache(
			cache_manager,
			name = paste0("CreateComponents_native"),
			hash = .make_hash(x, analyses, parameters, engine$Workflow),
			description = "Components created with CreateComponents_native method",
