...
#' @export
#'
MassSpecMethod_FeatureBlankSubtraction_native <- function(
	blankThreshold = 5,
	rtExpand = 10,
	mzExpand = 0.005
) {
	...
	if (is.null(validate_object(x))) {
		...
		stop("Invalid parameters for MassSpecMethod_FeatureBlankSubtraction_native.")
	}
}

#' @export
#' @noRd
validate_object.MassSpecMethod_FeatureBlankSubtraction_native <- function(x) {
...
#' @export
#' @noRd
run.MassSpecMethod_FeatureBlankSubtraction_native <- function(x, engine = NULL) {
	...
	if (nrow(analyses) == 0) {
		warning("No analyses found in the MassSpecResults_NonTargetAnalysis.")
		return(FALSE)
	}

	blks <- unique(analyses$blank)
	if (length(blks) == 0 || all(is.na(blks))) {
		warning("No blanks defined in the MassSpecResults_NonTargetAnalysis.")
		return(FALSE)
	}
	...
	if (nrow(fts) == 0) {
		warning("No features found in MassSpecResults_NonTargetAnalysis.")
		return(FALSE)
	}
	...
	fts_list <- rcpp_nts_blank_subtraction(
		info = analyses_db,
	...
	if (!is.null(cache_manager)) {
		save_cache(
			cache_manager,
			name = paste0("FeatureBlankSubtraction_native"),
			hash = .make_hash(x, analyses, parameters, engine$Workflow),
			description = "Features found with FeatureBlankSubtraction_native method",
