#' DB_MassSpecMethod_FilterInternalStandards_native Class
#'
#' @description Settings for filtering internal standards in DB_MassSpecResults_NonTargetAnalysis objects based on internal standard properties.
#'
...
#'
#' @return A `DB_MassSpecMethod_FilterInternalStandards_native` object.
#'
#' @export
#'
DB_MassSpecMethod_FilterInternalStandards_native <- function(
  names = character(0),
  minScore = NA_real_,
  maxErrorRT = NA_real_,
  ...
  if (is.null(validate_object(x))) {
    ...
    stop("Invalid DB_MassSpecMethod_FilterInternalStandards_native object!")
  }
}
...
#' @noRd
#'
validate_object.DB_MassSpecMethod_FilterInternalStandards_native <- function(x) {
...
#' @noRd
#'
run.DB_MassSpecMethod_FilterInternalStandards_native <- function(
  x,
  engine = NULL
) {
  ...
  if (nrow(istd) == 0) {
    warning("No internal standards found in DB_MassSpecResults_NonTargetAnalysis! Not done.")
    return(FALSE)
  }
  ...
  istd_list <- rcpp_nts_filter_internal_standards_2(
    info = analyses_db,
  ...
  if (!is.null(cache_manager)) {
    save_cache(
      cache_manager,
      name = paste0("DB_FilterInternalStandards_native"),
      hash = .make_hash(x, analyses_info, parameters, engine$Workflow),
      description = "Internal standards filtered with DB_FilterInternalStandards_native method",
