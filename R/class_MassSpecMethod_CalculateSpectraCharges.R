...
MassSpecMethod_CalculateSpectraCharges_native <- function(
  ...
  if (is.null(validate_object(x))) {
    ...
    stop("Invalid MassSpecMethod_CalculateSpectraCharges_native object!")
  }
}
...
validate_object.MassSpecMethod_CalculateSpectraCharges_native <- function(x) {
  ...
  checkmate::assert_logical(x$parameters$onlyTopScans, max.len = 1)
  checkmate::assert_numeric(x$parameters$topScans, max.len = 1)
  checkmate::assert_number(x$parameters$roundVal)
  checkmate::assert_number(x$parameters$relLowCut)
  checkmate::assert_number(x$parameters$absLowCut)
...
#' @export
#' @noRd
run.MassSpecMethod_CalculateSpectraCharges_native <- function(
  x,
  engine = NULL
) {
  ...
  charges <- lapply(
    spec_list,
    function(z, roundVal, relLowCut, absLowCut, top_charges, onlyTopScans) {
      mz <- NULL
      . <- NULL
      if (nrow(z) > 0) {
        intensity <- NULL
        ...
        if (nrow(sp2) < 3) {
          warning(
            "Not enough clusters in analysis ",
            processed,
        ...
        find_outliers <- function(res, main_cluster_position, main_mz_charge) {
          ...
          clusters_right_side <- main_cluster_position +
            seq_len(nrow(res) - main_cluster_position)
        ...
        for (main_mz in top_3_mz) {
          ...
          if (main_charge_right != main_charge_left) {
            res_left <- find_outliers(
              res,
              main_cluster_position,
              main_charge_left
            )
            res_right <- find_outliers(
              res,
              main_cluster_position,
              main_charge_right
            )
        ...
        plot_charges_annotated <- function(z, res) {
          ...
          text(
            x = res$mz[no_outlier],
            y = res$intensity[no_outlier],
            adj = c(-0.1, 0.25),
            labels = paste0(
              round(res$mz[no_outlier], digits = 3),
              " (+",
              res$z[no_outlier],
              ")"
            ),
          ...
          if (any(res$outlier)) {
            text(
              x = res$mz[is_outlier],
              y = res$intensity[is_outlier],
              adj = c(-0.1, 0.25),
              labels = paste0(
                round(res$mz[is_outlier], digits = 3),
                " (+",
                res$z[is_outlier],
                ")"
              ),
        ...
        for (i in seq_len(nrow(res))) {
          res$mass[i] <- calculate_mass(res$mz[i], res$z[i])
        }
  ...
  spectra <- lapply(charges, function(z) z$spectra)
  charges <- lapply(charges, function(z) z$charges)
  spec_obj$spectra <- spectra
  spec_obj$charges <- charges
