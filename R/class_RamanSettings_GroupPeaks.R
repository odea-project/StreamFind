...
#'
#' @return A RamanSettings_GroupPeaks_native object.
#'
#' @export
#'
RamanSettings_GroupPeaks_native <- S7::new_class(
  "RamanSettings_GroupPeaks_native",
...
#' @export
#' @noRd
S7::method(run, RamanSettings_GroupPeaks_native) <- function(x, engine = NULL) {
  ...
  for (p in seq_len(nrow(chrom_peaks))) {
    ...
    group_name <- paste0(group_number, "_", round(mean(temp$rt), digits = 0))
