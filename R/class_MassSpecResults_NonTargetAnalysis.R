...
MassSpecResults_NonTargetAnalysis <- function(
  ...
  analyses_db <- file.path(projectPath, "MassSpecAnalyses.duckdb")
  if (!file.exists(analyses_db)) stop("MassSpecAnalyses.duckdb not found at projectPath: ", analyses_db)
  analyses_obj <- MassSpecAnalyses(projectPath = projectPath)

  db <- file.path(projectPath, "MassSpecResults_NonTargetAnalysis.duckdb")
  dir.create(dirname(db), recursive = TRUE, showWarnings = FALSE)
  conn <- DBI::dbConnect(duckdb::duckdb(), db)
  on.exit(DBI::dbDisconnect(conn), add = TRUE)
  .create_MassSpecResults_NonTargetAnalysis_Features_db_schema(conn)
  .validate_MassSpecResults_NonTargetAnalysis_Features_db_schema(conn)
  .create_MassSpecResults_NonTargetAnalysis_InternalStandards_db_schema(conn)
  .validate_MassSpecResults_NonTargetAnalysis_InternalStandards_db_schema(conn)
  .create_MassSpecResults_NonTargetAnalysis_Suspects_db_schema(conn)
  .validate_MassSpecResults_NonTargetAnalysis_Suspects_db_schema(conn)
  .create_MassSpecResults_NonTargetAnalysis_TransformationProducts_db_schema(conn)
  .validate_MassSpecResults_NonTargetAnalysis_TransformationProducts_db_schema(conn)

  insert_features <- function(features) {
    .validate_MassSpecResults_NonTargetAnalysis_features_dt(features)
...
validate_object.MassSpecResults_NonTargetAnalysis <- function(x) {
  checkmate::assert_class(x, "MassSpecResults_NonTargetAnalysis")
  checkmate::assert_true(identical(x$dataType, "MassSpec"))
  if (!file.exists(x$db)) stop("MassSpecResults_NonTargetAnalysis file not found: ", x$db)
  if (is.null(x$analyses) || !inherits(x$analyses, "MassSpecAnalyses")) {
    stop("Field analyses must be a MassSpecAnalyses object.")
  }
  ...
  if (!all(required_tables %in% present)) stop("Missing required tables in MassSpecResults_NonTargetAnalysis")
  .validate_MassSpecResults_NonTargetAnalysis_Features_db_schema(conn)
  NextMethod()
}

# MARK: query_db (dispatch to base)
#' @describeIn MassSpecResults_NonTargetAnalysis Internal: execute a query on the DB (delegates to Results).
#' @template arg-ntsdb-x
#' @template arg-sql-sql
#' @template arg-sql-params
#' @export
query_db.MassSpecResults_NonTargetAnalysis <- function(x, sql, params = NULL) {
  NextMethod()
}

# MARK: list_db_tables (dispatch to base)
#' @describeIn MassSpecResults_NonTargetAnalysis Internal: list tables in the DB (delegates to Results).
#' @template arg-ntsdb-x
#' @export
list_db_tables.MassSpecResults_NonTargetAnalysis <- function(x) {
  NextMethod()
}

# MARK: get_db_table_info (dispatch to base)
#' @describeIn MassSpecResults_NonTargetAnalysis Internal: get table info from the DB (delegates to Results).
#' @template arg-ntsdb-x
#' @template arg-sql-tableName
#' @export
get_db_table_info.MassSpecResults_NonTargetAnalysis <- function(x, tableName) {
  NextMethod()
}
...
# MARK: get_features
#' @describeIn MassSpecResults_NonTargetAnalysis Retrieves features from the MassSpecResults_NonTargetAnalysis object based on specified criteria.
#' @template arg-ntsdb-x
#' @template arg-analyses
...
#' @describeIn MassSpecResults_NonTargetAnalysis Returns a data table with feature-group profiles across analyses.
#' @template arg-ntsdb-x
#' @template arg-analyses
#' @template arg-ms-groups
#' @template arg-ms-mass
...
#' @export
#'
get_features_profile.MassSpecResults_NonTargetAnalysis <- function(
  x,
  analyses = NULL,
  groups = NULL,
  mass = NULL,
...
#' @describeIn MassSpecResults_NonTargetAnalysis Plots feature-group profiles across analyses or replicates.
#' @template arg-ntsdb-x
#' @template arg-analyses
#' @template arg-ms-groups
#' @template arg-ms-mass
...
#' @export
#'
plot_features_profile.MassSpecResults_NonTargetAnalysis <- function(
  x,
  analyses = NULL,
  groups = NULL,
  mass = NULL,
...
#' @describeIn MassSpecResults_NonTargetAnalysis Retrieves internal standards from the InternalStandards table.
#' @template arg-ntsdb-x
#' @template arg-analyses
#' @export
#'
...
# MARK: map_features
#' @describeIn MassSpecResults_NonTargetAnalysis Plot RT vs m/z traces for selected features using EIC data.
#' @template arg-ntsdb-x
#' @template arg-analyses
#' @template arg-ms-features
#' @template arg-ms-groups
...
#' @describeIn MassSpecResults_NonTargetAnalysis Plot MS1 spectra for selected features.
#' @template arg-ntsdb-x
#' @template arg-analyses
#' @template arg-ms-features
#' @template arg-ms-groups
...
#' @describeIn MassSpecResults_NonTargetAnalysis Plot MS2 spectra for selected features.
#' @template arg-ntsdb-x
#' @template arg-analyses
#' @template arg-ms-features
#' @template arg-ms-groups
...
#' @describeIn MassSpecResults_NonTargetAnalysis Performs suspect screening on features stored in the DuckDB
...
#' @export
#'
suspect_screening.MassSpecResults_NonTargetAnalysis <- function(
  x,
  analyses = NULL,
  suspects = data.frame(),
  ppm = 5,
  ...
  suspects_out <- rcpp_nts_suspect_screening(
    info = analyses_info,
...
# MARK: get_suspects
#' @describeIn MassSpecResults_NonTargetAnalysis Retrieves suspect screening results from the Suspects table in the database.
#' @template arg-ntsdb-x
#' @template arg-analyses
#' @template arg-ms-features
#' @template arg-ms-groups
...
#' @describeIn MassSpecResults_NonTargetAnalysis Plot suspect MS2 spectra for selected features.
#' @template arg-ntsdb-x
#' @template arg-analyses
#' @template arg-ms-features
#' @template arg-ms-groups
...
#' @export
#'
plot_suspects_ms2.MassSpecResults_NonTargetAnalysis <- function(
  x,
  analyses = NULL,
  features = NULL,
  groups = NULL,
...
# MARK: get_internal_standards
#' @describeIn MassSpecResults_NonTargetAnalysis Retrieves internal standards from the database.
#' @template arg-ntsdb-x
#' @template arg-analyses
#' @export
#'
get_internal_standards.MassSpecResults_NonTargetAnalysis <- function(x, analyses = NULL) {
  conn <- DBI::dbConnect(duckdb::duckdb(), x$db)
  on.exit(DBI::dbDisconnect(conn), add = TRUE)
...
#' @describeIn MassSpecResults_NonTargetAnalysis Gets a data.table with fold-change analysis between the
...
#' @describeIn MassSpecResults_NonTargetAnalysis Plots the fold-change analysis between the `replicatesIn`
...
#' @describeIn MassSpecResults_NonTargetAnalysis Retrieves transformation products from the TransformationProducts table in the database.
#' @template arg-ntsdb-x
...
#' @export
#'
get_transformation_products.MassSpecResults_NonTargetAnalysis <- function(x, parents = NULL, groups = NULL) {
  conn <- DBI::dbConnect(duckdb::duckdb(), x$db)
  on.exit(DBI::dbDisconnect(conn), add = TRUE)
...
#' @describeIn MassSpecResults_NonTargetAnalysis Plot a transformation products network.
...
#' @export
#'
plot_transformation_products.MassSpecResults_NonTargetAnalysis <- function(x, groups = NULL, showMS2 = FALSE, showIntensityProfile = FALSE) {
...
# MARK: .validate_MassSpecResults_NonTargetAnalysis_features_dt
#' @noRd
.validate_MassSpecResults_NonTargetAnalysis_features_dt <- function(x) {
  cols <- c(
...
# MARK: .create_MassSpecResults_NonTargetAnalysis_Features_db_schema
#' @noRd
.create_MassSpecResults_NonTargetAnalysis_Features_db_schema <- function(conn) {
...
# MARK: .validate_MassSpecResults_NonTargetAnalysis_Features_db_schema
#' @noRd
.validate_MassSpecResults_NonTargetAnalysis_Features_db_schema <- function(conn) {
  tryCatch(
    {
...
# MARK: .create_MassSpecResults_NonTargetAnalysis_InternalStandards_db_schema
#' @noRd
.create_MassSpecResults_NonTargetAnalysis_InternalStandards_db_schema <- function(conn) {
...
# MARK: .validate_MassSpecResults_NonTargetAnalysis_InternalStandards_db_schema
#' @noRd
.validate_MassSpecResults_NonTargetAnalysis_InternalStandards_db_schema <- function(conn) {
  tryCatch(
    {
...
# MARK: .create_MassSpecResults_NonTargetAnalysis_TransformationProducts_db_schema
#' @noRd
.create_MassSpecResults_NonTargetAnalysis_TransformationProducts_db_schema <- function(conn) {
...
# MARK: .validate_MassSpecResults_NonTargetAnalysis_TransformationProducts_db_schema
#' @noRd
.validate_MassSpecResults_NonTargetAnalysis_TransformationProducts_db_schema <- function(conn) {
  tryCatch(
    {
      if (!"TransformationProducts" %in% DBI::dbListTables(conn)) {
        .create_MassSpecResults_NonTargetAnalysis_TransformationProducts_db_schema(conn)
...
# MARK: .create_MassSpecResults_NonTargetAnalysis_Suspects_db_schema
#' @noRd
.create_MassSpecResults_NonTargetAnalysis_Suspects_db_schema <- function(conn) {
...
# MARK: .validate_MassSpecResults_NonTargetAnalysis_Suspects_db_schema
#' @noRd
.validate_MassSpecResults_NonTargetAnalysis_Suspects_db_schema <- function(conn) {
  tryCatch(
    {
