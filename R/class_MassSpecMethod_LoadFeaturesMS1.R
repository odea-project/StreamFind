...
#' @title MassSpecMethod_LoadFeaturesMS1_native class
...
#' @export
#'
MassSpecMethod_LoadFeaturesMS1_native <- function(
  rtWindow = c(-2, 2),
  mzWindow = c(-1, 6),
  mzClust = 0.005,
  presence = 0.8,
  ...
  if (is.null(validate_object(x))) {
    ...
    stop("Invalid parameters for MassSpecMethod_LoadFeaturesMS1_native.")
  }
}

#' @export
#' @noRd
validate_object.MassSpecMethod_LoadFeaturesMS1_native <- function(x) {
...
#' @export
#' @noRd
run.MassSpecMethod_LoadFeaturesMS1_native <- function(x, engine = NULL) {
  ...
  fts <- rcpp_nts_load_features_ms1(
    info = analyses,
  ...
  if (!is.null(cache_manager)) {
    save_cache(
      cache_manager,
      name = paste0("LoadFeaturesMS1_native"),
      hash = .make_hash(x, analyses, parameters, engine$Workflow),
      description = "MS1 loaded with LoadFeaturesMS1_native method",
