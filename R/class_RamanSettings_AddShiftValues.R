...
#'
#' @return A RamanSettings_AddShiftValues_native object.
#'
#' @export
#'
RamanSettings_AddShiftValues_native <- S7::new_class(
  "RamanSettings_AddShiftValues_native",
  ...
  validator = function(self) {
    ...
    checkmate::assert_integer(as.integer(self@parameters$index))
    checkmate::assert_numeric(self@parameters$shifts)
    NULL
  }
)
...
#' @export
#' @noRd
S7::method(run, RamanSettings_AddShiftValues_native) <- function(x, engine = NULL) {
  ...
  if (algo_mode %in% "analyses") {
    ...
    if (is.na(index)) index <- seq_len(length(spec_list))
  ...
  spec_list <- lapply(spec_list, function(z, shifts) {
