...
RamanEngine <- R6::R6Class("RamanEngine",
  ...
  active = list(
    ...
    # MARK: RamanResults_Spectra
    #' @field RamanResults_Spectra `RamanResults_Spectra` results object for each analysis or replicate.
    RamanResults_Spectra = function(value) {
      if (missing(value)) {
        return(self$Analyses$RamanResults_Spectra)
      }
      self$Analyses$RamanResults_Spectra <- value
      invisible(self)
    }
  ),
  ...
  public = list(
    ...
    initialize = function(metadata = NULL,
                          workflow = NULL,
                          analyses = NULL) {
      super$initialize(metadata, workflow, analyses, type = "Raman")
      invisible(self)
    },
    ...
    #' @param analyses A `character vector` with full file paths to "asc", "sif", "json", "wdf", "sdf", "csv" and/or "txt" raman files.
    #'
