...
#'
#' @return A MassSpecSettings_FindChromPeaks_LocalMaxima object.
#'
#' @export
#'
...
#' @export
#' @noRd
S7::method(run, MassSpecSettings_FindChromPeaks_LocalMaxima) <- function(x, engine = NULL) {
  ...
  chrom_peaks <- lapply(chroms, function(s) {
    ...
    for (uid in uids) {
      ...
      if (FALSE) {
        plot(s_id$rt, s_id$intensity, type = "l", main = "Chromatogram with Relevant Peaks")
      ...
      if (nrow(peaks_id) > 0) {
        peak_results <- data.table::rbindlist(list(peak_results, peaks_id), fill = TRUE)
      }
    }
...
#' **MassSpecSettings_FindChromPeaks_pracma**
#'
...
#'
#' @return A MassSpecSettings_FindChromPeaks_pracma object.
#'
#' @export
#'
MassSpecSettings_FindChromPeaks_pracma <- S7::new_class(
  name = "MassSpecSettings_FindChromPeaks_pracma",
  parent = ProcessingSettings,
  package = "StreamFind",
...
#' @export
#' @noRd
S7::method(run, MassSpecSettings_FindChromPeaks_pracma) <- function(x, engine = NULL) {
