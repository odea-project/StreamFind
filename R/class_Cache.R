...
#' @description S3 class to manage `Cache.duckdb` for caching and loading data.
#' @template arg-projectPath
#' @return An object of class `Cache`.
#' @export
Cache <- function(projectPath = ".") {
  if (!requireNamespace("duckdb", quietly = TRUE)) {
    stop("duckdb package is required for Cache")
  }
  db <- file.path(projectPath, "Cache.duckdb")
...
clear_cache.Cache <- function(x, value = NULL, ...) {
  ...
  if (is.null(value)) {
    ...
    if (file.remove(x$db)) {
      Cache(projectPath = dirname(x$db))
      invisible(TRUE)
    } else {
...
# MARK: query_db
#' @describeIn Cache Internal: execute a query on the DB.
#' @param x Cache object
#' @param sql SQL query string
#' @param params Query parameters (optional)
#' @export
query_db.Cache <- function(x, sql, params = NULL) {
  conn <- DBI::dbConnect(duckdb::duckdb(), x$db)
  on.exit(DBI::dbDisconnect(conn), add = TRUE)
...
# MARK: list_db_tables
#' @describeIn Cache Internal: list tables in the DB.
#' @param x Cache object
#' @export
list_db_tables.Cache <- function(x) {
  conn <- DBI::dbConnect(duckdb::duckdb(), x$db)
  on.exit(DBI::dbDisconnect(conn), add = TRUE)
  .list_db_tables(conn)
}
...
#' @describeIn Cache Internal: get table info from the DB.
#' @param x Cache object
#' @param tableName Table name
#' @export
get_db_table_info.Cache <- function(x, tableName) {
  conn <- DBI::dbConnect(duckdb::duckdb(), x$db)
  on.exit(DBI::dbDisconnect(conn), add = TRUE)
