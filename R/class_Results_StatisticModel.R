...
StatisticModel <- S7::new_class("StatisticModel", package = "StreamFind", parent = Results,
  ...
  properties = list(
    ...
    ## __has_model -----
    has_model = S7::new_property(S7::class_list, default = list(),
      getter = function(self) {
        if (length(self$model) == 0) return(FALSE)
        TRUE
      }
    ),
    ...
    ## __has_test -----
    has_test = S7::new_property(S7::class_list, default = list(),
      getter = function(self) {
        if (length(self$model$res$test) == 0) return(FALSE)
        TRUE
      }
    ),
    ...
    ## __has_prediction -----
    has_prediction = S7::new_property(S7::class_list, default = list(),
      getter = function(self) {
        if (length(self$model$res$prediction) == 0) return(FALSE)
        TRUE
      }
    ),
    ...
    prediction = S7::new_property(S7::class_list, default = list(),
      getter = function(self) {
        if (length(self$model$res$prediction) == 0) return(NULL)
...
#' @export
#' @noRd
MCRALS <- S7::new_class("MCRALS", package = "StreamFind", parent = StatisticModel,
