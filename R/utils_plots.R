...
.plot_spectra_3d_interactive <- function(spectra = NULL,
  ...
  hover_text <- paste0(
    ...
    "<br>level: ", spectra$level,
    "<br>polarity: ", spectra$polarity,
    "<br>rt: ", spectra$rt,
    "<br>mz: ", spectra$mz,
    "<br>drift: ", spectra$drift,
...
# MARK: .plot_spectra_eic_interactive
#' @noRd
.plot_spectra_eic_interactive <- function(eic = NULL,
  ...
  for (t in loop_key) {
    ...
    plot <- plot %>% add_trace(
      ...
      showlegend = showL[lt],
      hovertemplate = paste(
        "<br>id: ", eic$id[select_vector],
        "<br>analysis: ", eic$analysis[select_vector],
        "<br>replicate: ", eic$replicate[select_vector],
        "<br>rt: %{x}",
        if ("mz" %in% colnames(eic)) {
          paste("<br>mz: ", round(eic$mz[select_vector], digits = 4))
        },
        "<br>intensity: %{y}"
      )
    )
...
# MARK: .plot_features_interactive
#' @noRd
...
# MARK: .map_components_interactive
#' @noRd
...
# MARK: .plot_internal_standards_qc_interactive
#' @noRd
...
# MARK: .plot_chromatograms_interactive
#' @noRd
...
# MARK: .plot_suspects_interactive
#' @noRd
...
.plot_spec_charges_static <- function(spec = NULL,
  ...
  for (t in ids) {
    ...
    text(
      x = pk$mz, y = pk$intensity, adj = c(-0.1, 0.25),
...
# MARK: .plot_spec_charges_interactive
#' @noRd
...
# MARK: .plot_chrom_peaks_interactive
#' @noRd
...
# MARK: .plot_x_spectra_baseline_static
#' @noRd
...
# MARK: .plot_x_spectra_baseline_interactive
#' @noRd
