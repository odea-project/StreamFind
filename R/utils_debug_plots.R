...
#'
#' @param logFile Path to the debug log file. Default: "log/debug_log_group_features_*.log"
#' @return A plotly figure object showing RT shifts per analysis
#' @keywords internal
#' @noRd
.plot_debug_DB_MassSpecMethod_GroupFeatures_native <- function(logFile) {

  # Check if plotly is available
  if (!requireNamespace("plotly", quietly = TRUE)) {
    stop("Package 'plotly' is required for debug plotting. Install it with: install.packages('plotly')")
  }

  # Check if log file exists
  if (!file.exists(logFile)) {
    stop("Log file not found: ", logFile)
  }

  # Read the log file
  log_lines <- readLines(logFile, warn = FALSE)

  # Find the RT Shift Vectors section
  shift_section_start <- grep("--- RT Shift Vectors \\(for plotting\\) ---", log_lines, value = FALSE)

  if (length(shift_section_start) == 0) {
    stop("No RT Shift Vectors section found in log file. Make sure debug=TRUE was used.")
  }

  # Parse the RT shift data for each analysis
  shift_data_list <- list()

  # Find all analysis headers
  analysis_lines <- grep("^# Analysis:", log_lines, value = FALSE)

  for (line_idx in analysis_lines) {
    # Extract analysis name
    analysis_line <- log_lines[line_idx]
    analysis_name <- sub("^# Analysis: ", "", analysis_line)

    # Read rt_vector statement (may span multiple lines)
    rt_line_complete <- ""
    current_idx <- line_idx + 1
    while (current_idx <= length(log_lines)) {
      line <- log_lines[current_idx]
      if (grepl("^rt_vector <-", line)) {
        rt_line_complete <- line
        current_idx <- current_idx + 1
        # Continue reading until we find the closing parenthesis
        while (current_idx <= length(log_lines) && !grepl("\\)\\s*$", rt_line_complete)) {
          rt_line_complete <- paste0(rt_line_complete, log_lines[current_idx])
          current_idx <- current_idx + 1
        }
        break
      }
    ...
    # Read shift_vector statement (may span multiple lines)
    shift_line_complete <- ""
    while (current_idx <= length(log_lines)) {
      line <- log_lines[current_idx]
      if (grepl("^shift_vector <-", line)) {
        shift_line_complete <- line
        current_idx <- current_idx + 1
        # Continue reading until we find the closing parenthesis
        while (current_idx <= length(log_lines) && !grepl("\\)\\s*$", shift_line_complete)) {
          shift_line_complete <- paste0(shift_line_complete, log_lines[current_idx])
          current_idx <- current_idx + 1
        }
        break
      }
    ...
    # Parse the vectors
    if (nzchar(rt_line_complete) && nzchar(shift_line_complete)) {
      # Evaluate the R code to get vectors
      rt_vector <- NULL
      shift_vector <- NULL

      tryCatch({
        eval(parse(text = rt_line_complete))
        eval(parse(text = shift_line_complete))

        if (!is.null(rt_vector) && !is.null(shift_vector) &&
            length(rt_vector) == length(shift_vector)) {
          shift_data_list[[analysis_name]] <- data.frame(
        ...
        warning("Failed to parse RT shift data for analysis: ", analysis_name,
                " - Error: ", conditionMessage(e))
      })
    }
  }

  if (length(shift_data_list) == 0) {
    stop("No valid RT shift data found in log file.")
  }

  # Combine all data
  all_shifts <- do.call(rbind, shift_data_list)

  # Remove NaN shifts for plotting
  all_shifts <- all_shifts[!is.na(all_shifts$shift), ]

  if (nrow(all_shifts) == 0) {
    stop("No valid (non-NaN) shift data found in log file.")
  }

  # Create plotly line plot
  fig <- plotly::plot_ly()

  # Add a trace for each analysis
  analyses <- unique(all_shifts$analysis)
  colors <- grDevices::rainbow(length(analyses))

  for (i in seq_along(analyses)) {
    analysis_name <- analyses[i]
    analysis_data <- all_shifts[all_shifts$analysis == analysis_name, ]

    fig <- plotly::add_trace(
      fig,
      x = analysis_data$rt,
      y = analysis_data$shift,
      type = "scatter",
      mode = "lines+markers",
      name = analysis_name,
      line = list(color = colors[i], width = 2),
      marker = list(size = 4, color = colors[i]),
      text = paste0("Analysis: ", analysis_name,
                    "<br>RT: ", analysis_data$rt, "s",
                    "<br>Shift: ", round(analysis_data$shift, 3), "s"),
      hoverinfo = "text"
    )
  }
  ...
  fig <- plotly::layout(
    fig,
    title = list(
      text = paste0("<b>RT Alignment Shifts</b><br>",
                   "<sub>", basename(logFile), "</sub>"),
      x = 0.5,
      xanchor = "center"
    ),
    xaxis = list(
...
#' Plot cluster data from DB_MassSpecMethod_FindFeatures_native debug log
#'
#' @param logFile Path to the debug log file. Default: "log/debug_log_peak_detection_*.log"
#' @param plot3D Logical. If TRUE, creates 3D plot. If FALSE (default), creates 2D line plot.
#' @return A plotly figure object
#' @keywords internal
#' @noRd
.plot_debug_DB_MassSpecMethod_FindFeatures_native <- function(logFile, plot3D = FALSE) {
