#' @noRd
#' @export
.mod_WADB_Explorer_UI.DB_MassSpecAnalyses <- function(x, id, ns) {
  ns2 <- shiny::NS(id)
      ...
      shiny::fluidRow(
        shiny::column(
          ...
          DT::dataTableOutput(
            ...
            height = "calc(100vh - 60px - 10px - 44px)"
          )
        ),
        shiny::column(9,
...
#' @noRd
#' @export
.mod_WADB_Explorer_Server.DB_MassSpecAnalyses <- function(
    x,
    id,
    ns,
    reactive_analyses,
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    update_reactive_vars <- function(
      ...
      analyses_info <- info(reactive_analyses())
      ...
      hd <- get_spectra_headers(reactive_analyses())
      if (nrow(hd) > 0) {
    ...
    output$spectraAnalysesTable <- DT::renderDT({
      ...
      DT::datatable(
        ...
        options = list(
          dom = "ft",
          paging = FALSE,
          scrollY = "calc(100vh - 60px - 10px - 44px - 10px - 100px)",
          scrollCollapse = TRUE
        )
      )
    })
          ...
          if (as.logical(input$summary_plot_interactive)) {
            shinycssloaders::withSpinner(
              plotly::plotlyOutput(
                ...
                height = "calc(100vh - 60px - 10px - 44px - 30px)"
              ),
              color = "black"
            )
          } else {
