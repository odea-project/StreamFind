...
MassSpecTargets <- function(
  ...
  if (is.data.frame(mass)) {
    checkmate::assert_true(
      cols_mass %in% colnames(mass) || all(cols_mass_ranges %in% colnames(mass))
    )
    targets <- mass
  ...
  if (is.data.frame(rt)) {
    checkmate::assert_true(
      cols_rt %in% colnames(rt) || all(cols_rt_ranges %in% colnames(mz))
    )
    ...
    if (is.vector(rt)) {
      if (is.null(targets)) {
        ...
        if (length(rt) == nrow(targets)) {
          ...
          warning(
            "RT vector length has different length and the number of rows in targets!"
          )
        }
      }
    } else {
  ...
  if (is.data.frame(mobility)) {
    ...
    if (is.null(targets)) {
      ...
      if (nrow(mobility) == nrow(targets)) {
        ...
        warning(
          "Mobility data frame has different number of rows than targets!"
        )
      }
    }
    ...
    if (is.vector(mobility)) {
      if (is.null(targets)) {
        ...
        if (length(mobility) == nrow(targets)) {
          ...
          warning(
            "Mobility vector length has different length and the number of rows in targets!"
          )
        }
      }
    } else {
  ...
  if (!is.null(targets)) {
    if (is.null(polarities) && !"polarity" %in% colnames(targets)) {
      ...
      if (
        !is.null(polarities) &&
          "analysis" %in% colnames(targets) &&
          !is.null(names(polarities))
      ) {
    ...
    for (i in seq_len(nrow(targets))) {
      if (grepl("1", targets$polarity[i]) && grepl("-1", targets$polarity[i])) {
        if (
          cols_mz %in%
            colnames(targets) ||
            all(cols_mz_ranges %in% colnames(targets))
        ) {
          warning(
            "Polarity is both positive and negative, but m/z is given! Not possible to define polarity for target."
          )
          targets <- targets[, -i]
        } else {
    ...
    if (nrow(targets) > 0) {
      ...
      if (
        !cols_mass %in% colnames(targets) &&
          !cols_mz %in% colnames(targets) &&
          all(cols_mass_ranges %in% colnames(targets))
      ) {
        pols <- rep(1, nrow(targets))
        pols[targets$polarity == -1] <- -1
      ...
      if (
        cols_rt %in%
          colnames(targets) &&
          !all(cols_rt_ranges %in% colnames(targets))
      ) {
        if (is.null(sec)) {
          sec <- 0
        }
      ...
      if (
        cols_mobility %in%
          colnames(targets) &&
          !all(cols_mobility_ranges %in% colnames(targets))
      ) {
        if (is.null(millisec)) {
          millisec <- 0
        }
      ...
      if (
        !cols_mobility %in% colnames(targets) &&
          all(cols_mobility_ranges %in% colnames(targets))
      ) {
        targets$mobility <- apply(
          targets[, cols_mobility_ranges, with = FALSE],
          1,
          mean
        )
      }

      if (!cols_mz %in% colnames(targets)) {
        targets$mz <- 0
      }
      if (!cols_rt %in% colnames(targets)) {
        targets$rt <- 0
      }
      if (!cols_mobility %in% colnames(targets)) {
        targets$mobility <- 0
      }
      if (!cols_mz_ranges[1] %in% colnames(targets)) {
        targets$mzmin <- 0
      }
      if (!cols_mz_ranges[2] %in% colnames(targets)) {
        targets$mzmax <- 0
      }
      if (!cols_rt_ranges[1] %in% colnames(targets)) {
        targets$rtmin <- 0
      }
      if (!cols_rt_ranges[2] %in% colnames(targets)) {
        targets$rtmax <- 0
      }
      if (!cols_mobility_ranges[1] %in% colnames(targets)) {
        targets$mobilitymin <- 0
      }
      if (!cols_mobility_ranges[2] %in% colnames(targets)) {
        targets$mobilitymax <- 0
      }
      ...
      if (!is.null(id)) {
        if (length(id) == nrow(targets)) targets$id <- id
      }
