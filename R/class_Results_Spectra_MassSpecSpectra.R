...
#' @export
#' @noRd
`[.StreamFind::MassSpecSpectra` <- function(x, i) {
...
S7::method(get_spectra, MassSpecSpectra) <- function(x,
  ...
  polarities <- vapply(x$spectra, function(z) {
    if ("polarity" %in% colnames(z)) {
      pol <- unique(z$polarity)
      pol[pol == 0] <- "unkown"
      pol[pol == 1] <- "positive"
      pol[pol == -1] <- "negative"
      if (length(pol) > 1) pol <- paste(pol, collapse = ", ")
      pol
    } else {
      "unkown"
    }
  ...
  if (all(apply(targets[, num_cols, with = FALSE], 1, function(z) sum(z, na.rm = TRUE)) != 0)) {
    ...
    if (TRUE %in% is.na(targets$mzmax)) {
      targets$mzmax[is.na(targets$mzmax)] <- max(vapply(x$spectra, function(z) max(z$mz), 0))
    }
    if (TRUE %in% is.na(targets$mzmin)) {
      targets$mzmin[is.na(targets$mzmin)] <- min(vapply(x$spectra, function(z) min(z$mz), 0))
    }
    if (TRUE %in% (targets$mzmax == 0)) {
      targets$mzmax[targets$mzmax == 0] <- max(vapply(x$spectra, function(z) max(z$mz), 0))
    }
    if (TRUE %in% is.na(targets$rt)) {
      targets$rt[is.na(targets$rt)] <- 0
    }
    if (TRUE %in% is.na(targets$rtmax)) {
      targets$rtmax[is.na(targets$rtmax)] <- max(vapply(x$spectra, function(z) max(z$rt), 0))
    }
    if (TRUE %in% is.na(targets$rtmin)) {
      targets$rtmin[is.na(targets$rtmin)] <- min(vapply(x$spectra, function(z) min(z$rt), 0))
    }
    if (TRUE %in% (targets$rtmax == 0)) {
      targets$rtmax[targets$rtmax == 0] <- max(vapply(x$spectra, function(z) max(z$rt), 0))
    }
    if (TRUE %in% is.na(targets$mobility)) {
      targets$mobility[is.na(targets$mobility)] <- 0
    }
    
    if (any(vapply(x$spectra, function(z) "mobility" %in% colnames(z), FALSE))) {
  ...
  if (is.null(levels)) {
    levels <- vapply(x$spectra, function(z) {
  ...
  spec_list <- lapply(x$spectra, function(temp, rpl, analyses) {
    ...
    if ("level" %in% colnames(temp)) {
      ...
      temp <- temp[!(temp$intensity <= max(c(minIntensityMS1, minIntensityMS2))), ]
    }
...
#' @export
#' @noRd
S7::method(plot_spectra, MassSpecSpectra) <- function(x,
                                                      analyses = NULL,
                                                      levels = NULL,
                                                      mass = NULL,
                                                      mz = NULL,
  ...
  unique_key <- c("analysis", "replicate", "id", "polarity", "level", "loop", "var", "xval")
  unique_key <- unique_key[unique_key %in% colnames(spec)]
  intensity <- NULL
  spec <- spec[, .(intensity = max(intensity)), by = c(unique_key)]
  
  if (!"level" %in% colnames(spec)) spec$level <- NA_integer_
...
#' @export
#' @noRd
S7::method(plot_spectra_3d, MassSpecSpectra) <- function(x,
                                                         analyses = NULL,
                                                         levels = NULL,
                                                         mass = NULL,
                                                         mz = NULL,
  ...
  if (xVal == "mz" && (!"mz" %in% colnames(spec)) && "mass" %in% colnames(spec)) xVal <- "mass"
  if (yVal == "mz" && (!"mz" %in% colnames(spec)) && "mass" %in% colnames(spec)) yVal <- "mass"
  ...
  hover_text <- paste0(
    "<br>id: ", spec$id,
    "<br>analysis: ", spec$analysis,
    "<br>replicate: ", spec$replicate,
    "<br>level: ", spec$level,
    "<br>polarity: ", spec$polarity,
    "<br>rt: ", spec$rt,
    "<br>mz: ", spec$mz,
    "<br>mobility: ", spec$mobility,
    "<br>intensity: ", spec$intensity
  )
  
  fig <- plot_ly(spec, x = ~x, y = ~y, z = ~intensity) %>%
    group_by(spec$rtmz) %>%
    add_lines(color = ~var, colors = colors_var, hoverinfo = "text", text = hover_text)
...
#' @export
#' @noRd
S7::method(plot_spectra_charges, MassSpecSpectra) <- function(x,
                                                              analyses = NULL,
                                                              legendNames = NULL,
                                                              title = NULL,
                                                              colorBy = "analyses",
  ...
  if (!interactive) {
    ...
    plot <- ggplot2::ggplot(res, ggplot2::aes(x = mz, y = intensity, group = loop)) +
    ...
    plot <- plot +
      ggplot2::scale_y_continuous(expand = c(0, 0), limits = c(0, max(res$intensity) * 1.5)) +
...
#' @export
#' @noRd
S7::method(plot_spectra_peaks, MassSpecSpectra) <- function(x,
                                                            analyses = NULL,
                                                            legendNames = TRUE,
                                                            colorBy = "analyses",
                                                            xVal = "mz",
  ...
  if (!interactive) {
    ...
    for (i in seq_len(nrow(pks))) {
      ...
      temp <- dplyr::filter(
        spec,
        analysis %in% pk_analysis & replicate %in% pk_replicate & spec_id %in% pk_spec
      )
...
#' @export
#' @noRd
S7::method(plot_spectra_baseline, MassSpecSpectra) <- function(x,
                                                               analyses = NULL,
                                                               legendNames = TRUE,
                                                               colorBy = "analyses",
                                                               xVal = "mz",
