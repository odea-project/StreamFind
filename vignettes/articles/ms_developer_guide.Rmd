...
As shown, the constructor of a *ProcessingSettings* is a function always including `Settings_[module name]_[algorithm name]`; More details are given in the Semantics (\@ref(semantics)). Then, the *ProcessingSettings* can be directly added to the *MassSpecEngine*.
...
jsonlite::prettify(readLines("~/github/StreamFind/vignettes/articles/ffs.json"))
```

```{r import-settings, results='markup'}
ms$import_settings("~/github/StreamFind/vignettes/articles/ffs.json")
