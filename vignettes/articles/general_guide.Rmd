...
```{r concept-image, fig.cap="StreamFind Concept", out.width="100%", echo=FALSE}
knitr::include_graphics("figures/StreamFind_Concept.png")
```
...
A `ProcessingStep` object must always have the engine type, the processing method name, the name of the algorithm to be used, the origin software, the main developer name and contact as well as a link to further information and the DOI, when available. Lastly but not least, the parameters which is a flexible list of conditions to apply the algorithm during data processing.
