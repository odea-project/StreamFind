...
Internally, the engines (R6 classes) use seven central S3 classes:
...
Data processing engines are fundamentally reference classes with methods for project management. The [`Engine`](https://odea-project.github.io/StreamFind/reference/Engine.html) is the parent class of all other data specific engines (e.g. [`MassSpecEngine`](https://odea-project.github.io/StreamFind/reference/MassSpecEngine.html) and [`RamanEngine`](https://odea-project.github.io/StreamFind/reference/RamanEngine.html)). As parent, the `Engine` holds uniform functions across child data dedicated engines (e.g. managing the `Metadata`, recording the `AuditTrail` and applying a data processing `Workflow`).
...
Data specific engines have dedicated active fields to access the major S3 classes (i.e., `Metadata`, `Analyses`, `Workflow`, `Results`, `AuditTrail` and `Config`). For instance, the `Analyses` active field in the `RamanEngine` is used to access the ``RamanAnalyses` child class dedicated to RS.
...
get_analysis_names(raman$Analyses)

# Access the spectrum of the first analysis in the Analyses object
head(raman$Analyses$analyses[[1]]$spectra)
```

The methods for data access and visualization are also implemented as public S3 methods to the S3 classes. Below, the `plot_spectra()` method from the `RamanEngine` is used to plot the raw spectra from analyses 1 and 12. The available methods for each class is described in the documentation of each class (e.g., [`RamanAnalyses`](https://odea-project.github.io/StreamFind/reference/RamanAnalyses.html)).
