...
Internally, the engines (R6 classes) use six central [S7](https://github.com/RConsortium/S7) classes:
...
Data specific engines have dedicated active fields to access the data. For instance, the `Analyses` active field in the `RamanEngine` is used to access the raw spectra and any `Results`. Note that accessing properties of [S7](https://github.com/RConsortium/S7) classes should be done with `@` instead of `$`.
