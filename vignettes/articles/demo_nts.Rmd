...
The NTS object holds methods to obtain original objects from the [patRoon](https://github.com/rickhelmus/patRoon) package. For instance, the S4 class `features` or `featureGroups` objects can be obtained via the `get_patRoon_features` method of the *NTS* results. The `patRoon` package provides a comprehensive set of functions, as shown below. See more information in the [patRoon reference documentation](https://rickhelmus.github.io/patRoon/reference/index.html).

```{r pat-features}
# Native patRoon object
fGroups <- get_patRoon_features(ms$nts, filtered = FALSE, featureGroups = TRUE)
