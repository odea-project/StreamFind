...
The created features and feature groups can be inspected as `data.table` objects or plotted by dedicated methods in the *MassSpecEngine* or methods of the created Results class. Internally, the *MassSpecEngine* stores the results in the *MassSpecResults_NonTargetAnalysis* object, which can be accessed with `MassSpecEngine$MassSpecResults_NonTargetAnalysis`. Yet, the engine interface is recommended for accessing the results. However, the *MassSpecResults_NonTargetAnalysis* object can be used for more advanced operations, such as exporting the results to a database or other formats or use the native objects from other packages (e.g., [patRoon](https://github.com/rickhelmus/patRoon)) as we demonstrate further in this article.
...
fGroups <- get_patRoon_features(ms$MassSpecResults_NonTargetAnalysis, filtered = FALSE, featureGroups = TRUE)
