...
raman_workflow_settings <- list(
  ...
  Settings_correct_spectra_baseline_baseline(method = "als", args = list(lambda = 3, p = 0.1, maxit = 10)),
...
For evaluation of the differences between the BSA products with the fused data, a statistic analysis is performed via the statisticEngine.
...
stat <- StatisticEngine$new(data = fusedMat)
stat$make_model(Settings_make_model_mcrpure_mdatools(ncomp = 2))
stat$get_model_explained_variance()
