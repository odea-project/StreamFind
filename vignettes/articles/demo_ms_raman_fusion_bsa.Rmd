...
ms$plot_ms1(rt = mean(rbindlist(ms$chromatograms_peaks)$rt), sec = 0.1,
...
ms <- MassSpecEngine$new(files = ms_files[1:4], headers = list(name = "BSA Quality Evaluation Project"))
...
ps_dc <- Settings_calculate_spectra_charges_StreamFind(roundVal = 15, relLowCut = 0.2, absLowCut = 8000)
ms$calculate_spectra_charges(ps_dc)

ms$deconvolute_spectra(Settings_deconvolute_spectra_StreamFind())

ms$smooth_spectra(Settings_smooth_spectra_movingaverage(windowSize = 10))

ms$normalize_spectra(Settings_normalize_spectra_snv(liftTozero = TRUE))

ms$normalize_spectra(Settings_normalize_spectra_blockweight())

# ms$bin_spectra(Settings_bin_spectra_StreamFind(bins = list("rt" = 5, "mass" = 5)))
...
r_settings <- list(
  ...
  Settings_subtract_spectra_section_StreamFind(sectionWindow = c(0, 1.6)),
  ...
  Settings_normalize_spectra_snv(liftTozero = TRUE)
)
...
# raman$plot_spectra_baseline(rt = c(2.8, 3.2))

raman$plot_spectra(analyses = c(2:6), rt = c(2.8, 3.2))

# View(raman$get_results("spectra"))
