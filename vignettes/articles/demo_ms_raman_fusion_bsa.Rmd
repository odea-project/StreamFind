...
raman$plot_chromatograms(yLab = "Abbundance sum for each spectrum", colorBy = "replicates")
```

```{r ,fig.cap="Raw averaged Raman spectra for each BSA product between minute 5.5 and 8."}
raman$plot_spectra(rt = c(5.5, 8), colorBy = "replicates")
```
...
data_pca <- prcomp(mrSpec, center = TRUE, scale. = FALSE)
pcs <- data_pca$x
...
```{r}
training_data <- fusedMat[1:2, ]  # Replace this with your actual training data
test_data <- fusedMat[3:4, ]  # Replace this with your actual test data
labels <- rownames(fusedMat[1:2, ])  # Replace this with your actual labels for training data
