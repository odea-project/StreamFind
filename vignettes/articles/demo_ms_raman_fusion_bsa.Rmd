...
The fusion of MS and Raman data is performed by combining the processed data from both engines. The data is then merged into a single data frame for statistical evaluation using a StatisticEngine.
...
plot(stat$model, show.labels = TRUE) # plot method from mdatools
```
...
plot_resolved_spectra(stat$analyses)
```
