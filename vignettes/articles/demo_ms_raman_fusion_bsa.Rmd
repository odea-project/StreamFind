...
#main_dir <- "E:/example_files/isoft/BSA_products"
...
ms$Analyses <- set_replicate_names(
  ms$Analyses,
  sub("_\\d+$", "", get_analysis_names(ms$Analyses))
)

# Overview
info(ms$Analyses)[, c(1:2, 4, 5)]
```
...
plot_spectra_tic(ms$Analyses, colorBy = "replicates")
```
...
plot_chromatograms(
  ...
  colorBy = "replicates+targets", normalized = FALSE
)
```
...
The average retention time of the peaks was `r mean(data.table::rbindlist(ms$Results$MassSpecResults_Chromatograms$peaks)$rt)` seconds with a standard deviation of `r sd(data.table::rbindlist(ms$Results$MassSpecResults_Chromatograms$peaks)$rt)`. The following step is to estimate the *m/z* range of BSA for deconvolution of charges. The *m/z* range needs to be defined once and is equal to all other analyses if the compound of interest is BSA. As for the retention time, shifts might occur due to the chromatographic separation.

```{r rt-av}
# Average retention time of the main peak
rt_av <- mean(
  data.table::rbindlist(ms$Results$MassSpecResults_Chromatograms$peaks)$rt
)
rt_av
```
...
plot_spectra(ms$Results$MassSpecResults_Spectra, colorBy = "replicates")
```
...
raman$Analyses <- set_replicate_names(
  raman$Analyses,
  unique(get_replicate_names(ms$Analyses))[1:4]
)

info(raman$Analyses)[, c(1:2, 5)]
```

```{r correct-raman-rt, include=FALSE}
corr_spectra <- lapply(raman$Analyses$analyses, function(x) x$spectra)
...
raman$Analyses$analyses <- Map(function(x, y) {
  x$spectra <- y
  x
}, raman$Analyses$analyses, corr_spectra)
```
...
plot_spectra(raman$Analyses, rt = c(430, 440), colorBy = "analyses")
```
...
plot_spectra_baseline(raman$Analyses, colorBy = "replicates")
```

```{r raman-plot_spectra, fig.cap="Processed Raman spectra for each BSA product."}
plot_spectra(raman$Analyses, colorBy = "replicates")
```
...
```{r fusion}
ms_mat <- get_spectra_matrix(ms$Results[["MassSpecResults_Spectra"]])
...
summary(stat$Results$StatisticResults_PCA_mdatools$model) # summary method is from mdatools
```

```{r pca-results, fig.cap="Overview plot for the PCA model."}
plot_overview(stat$Results$StatisticResults_PCA_mdatools)
```
...
plot_loadings(
  stat$Results$StatisticResults_PCA_mdatools,
  colorKey = rep(c("MS", "Raman"), c(ncol(ms_mat), ncol(raman_mat)))
)
```
...
plot(stat$Results$StatisticResults_PCA_mdatools$model, show.labels = TRUE) # plot method from mdatools
```
...
summary(stat2$Results$StatisticResults_MCRPURE_mdatools$model)
```

```{r mcr-model, fig.cap="Overview plot of the MCR pure model."}
plot_overview(stat2$Results$StatisticResults_MCRPURE_mdatools)
```

```{r mcr-data}
# Gets the most pure feature for the component
get_model_data(stat2$Results$StatisticResults_MCRPURE_mdatools)$purity
```
