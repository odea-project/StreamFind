...
The StreamFind is an R package and can be used for data management, processing, visualization and reporting. This guide focuses on describing the framework behind StreamFind. The framework of StreamFind is centered around [R6](https://r6.r-lib.org/index.html) classes, serving as data processing engines for different types of data (e.g. Mass Spectrometry and Raman Spectroscopy data). The data processing workflow is assembled within the engine via S3 classes that harmonized processing settings across different methods and algorithms. The input is data specific analysis files or S3 classes. The following sections describe the processing engines behavior, the processing settings concept and the analysis input routine.  

# Engines

Processing engines are used to manage, process, visualize and report data. The `CoreEngine` is the parent class of all other data specific engines. As parent, the `CoreEngine` manages the project information via an S3 class `ProjectHeaders` and registers the audit trail as well as other uniform functions (e.g. adding and removing analysis) across child data dedicated engines.  
...
Note that when an empty `CoreEngine` is created, required `ProjectHeaders` are created with name, author, path and date. Yet, `ProjectHeaders` can be specified directly during creation of the `CoreEngine` via the argument `headers` or added to the engine as shown in \@ref(project-headers).  

## Project headers

The `ProjectHeaders` S3 class is meant to hold project information that can be used to identify the engine when for example, combining different engines, or to add extra information, such as description, etc. The user can add any kind of attribute but it must have length. Below, `ProjectHeaders` are created and added to the `CoreEngine`.

```{r}
headers <- ProjectHeaders(name = "Project Example", author = "Mr X",
...
The `ProcessingSettings` S3 class are use as instructions to assemble the data processing workflow within the engine. 
