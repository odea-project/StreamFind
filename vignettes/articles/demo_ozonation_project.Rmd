...
workflow <- StreamFind::Workflow(
  list(
    ...
    # Filter features using the StreamFind algorithm
    ...
    # The s/n is calculated using the CalculateFeaturesQuality
    MassSpecSettings_FilterFeatures_StreamFind(
      minSnRatio = 5
    ),
...
Then, the *Workflow* can be added to the *MassSpecEngine*. Note that the order will matter when the workflow is applied!
...
The method `MassSpecEngine$plot_suspects()` is used to inspect the suspect screening results. A second plot is added 
to compare the experimental fragmentation pattern (top) with the fragmentation pattern of the respective reference 
standard (down) added within the database. The `colorBy` argument can be set to *targets+replicates* to legend the plot 
with combined keys of suspect target names and analysis replicate names.
